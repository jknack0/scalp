...
async def replay_through_pipeline(
    ...
    for name in strategy_names:
        if name == "orb":
            strategies.append(ORBStrategy(config={
    ...
    print(f"\nReplaying {len(df)} bars through {[s.strategy_id for s in strategies]}...")
    ...
    for s in strategies:
        print(f"    [{s.strategy_id}] signals={s._signals_today}, bars={s._bars_processed}")
