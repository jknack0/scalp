...
Usage:
    python scripts/generate_phase4_report.py --start 2020-01-01 --end 2024-01-01
    python scripts/generate_phase4_report.py --start 2020-01-01 --end 2024-01-01 --strategy orb
    python scripts/generate_phase4_report.py --start 2020-01-01 --end 2024-01-01 --strategy vwap -q
"""
...
def _make_strategy_factory(config_cls, strategy_cls):
    """Create a strategy factory for CPCV."""
    def factory():
        hub = FeatureHub()
        cfg = config_cls()
        cfg.require_hmm_states = []
        return [strategy_cls(cfg, hub)]
    return factory
...
def _run_strategy(
    ...
    # 1. CPCV
    print("  Running CPCV...")
    factory = _make_strategy_factory(config_cls, strategy_cls)
    ...
    for strategy_name in STRATEGY_MAP:
        if strategy_name in strategies_to_run:
            ...
            summary, locked_params, daily_pnl = _run_strategy(
                strategy_name, config_cls, strategy_cls,
