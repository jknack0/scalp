"""Triple-barrier labeler for 1-minute bars.

Generates direction labels (DOWN=-1, UP=+1) from 1m OHLCV bars.
...
logger = get_logger("tick_predictor.labels_1m")

MES_TICK_SIZE = 0.25


@dataclass(frozen=True)
class TripleBarrierConfig1M:
    """Config for 1m triple barrier labels.

    Default: 6-tick TP (1.50 pts), 4-tick SL (1.00 pts), 15-min horizon.
    Volatility scaling adjusts barriers by realized vol / median vol.
    """
    vertical_barrier_bars: int = 15       # 15 minutes
    tp_ticks: int = 6                     # 6 ticks = 1.50 points
    sl_ticks: int = 4                     # 4 ticks = 1.00 points
    volatility_scale: bool = True
    vol_lookback_bars: int = 60           # 60 minutes = 1 hour
    min_vol_scale: float = 0.5
    max_vol_scale: float = 3.0
    tick_size: float = MES_TICK_SIZE


class TripleBarrierLabeler1M:
    """Generate triple-barrier labels from 1m bars."""

    def __init__(self, config: TripleBarrierConfig1M | None = None) -> None:
        self.config = config or TripleBarrierConfig1M()

    def generate_labels(self, start_date: str, end_date: str) -> pl.DataFrame:
        """Generate labels from year-partitioned 1m Parquet."""
    ...
    def generate_labels_from_bars(self, df: pl.DataFrame) -> pl.DataFrame:
        """Generate labels from a pre-loaded 1m bars DataFrame.

        Uses high/low of future bars to detect barrier touches intra-bar,
        not just close prices. This is more realistic for limit/stop orders.
        """
        cfg = self.config
        logger.info("labels_1m_bars_loaded", rows=len(df))
        ...
        # Triple barrier scan using high/low for intra-bar touches
        ...
        for i in range(n - 1):
            ...
            for j in range(i + 1, end_idx + 1):
                # Check if both barriers were hit in same bar
        ...
        for row in dist.iter_rows(named=True):
            ...
            logger.info("label_1m_distribution",
    ...
    def save_labels(self, df: pl.DataFrame, start_date: str, end_date: str,
        ...
        path = out_dir / f"labels_1m_{start_date}_{end_date}{suffix}.parquet"
        df.write_parquet(path)
        logger.info("labels_1m_saved", path=str(path), rows=len(df))
        return path

    def _load_bars(self, start_date: str, end_date: str) -> pl.DataFrame:
        """Load 1m bars from year-partitioned Parquet."""
        ...
        if not paths:
            raise FileNotFoundError(
                f"No 1m parquet files for {start_date} to {end_date}. "
                f"Build them first: python scripts/build/bars.py --freq 1m"
            )
